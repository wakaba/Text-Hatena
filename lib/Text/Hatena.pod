...
=item <a data-hatena-embed data-hatena-lang data-hatena-width data-hatena-height>

If the C<data-hatena-oembed> attribute is specified, the link is
expected to be expanded to an "embeded" representation (e.g. embedded
player widget).  If the value is C<movie>, a movie player is expected.
Otherwise, if the value is C<barcode>, the QR code of the link URL is
expected.  Otherwise, if the value is C<httpdetail>, the "detail"
widget of Hatena Bookmark for the link URL is expected.  Otherwise, if
the value is C<title>, the title of the linked page is expected.
Otherwise, no specific media is expected.
...
=item <a data-hatena-keyword>

The attribute value is the keyword the element is linked to.

=item <hatena-map lat lon width height type t link target>

Represent a C<map:> syntax text, if C<expand_map> option is false.
